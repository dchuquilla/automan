...
create or replace function tg_kilomentraje_semanal() returns trigger AS
$$
  DECLARE i auto%rowtype;
  DECLARE delta_horas numeric;
  DECLARE delta_km numeric;
  DECLARE km_semanal numeric;
begin
  select time NEW.updated_at - time OLD.updated_at into delta_horas;
  select time NEW.current_km - time OLD.current_km into delta_km;
